...
start() ->
	...
	lthrift_ets:createNewTable(self(), ?POOLTABLE_NAME),
	poolServer ! {create_pool,self(),?POOL_DYN_NAME,?POOL_DYN_CONNECT_NUM,rpcworker},
	lthrift_ets:setTableReleaseTime(?POOLTABLE_NAME, ?POOL_UPDATE_TIME).
	
create() ->
   receive
	   {create_pool,_From,PoolName,NumCon,Option} ->
             lthrift:start_link(PoolName, NumCon, Option),
			 _From ! done,
             create();
       {create_sub_pool,_From,Args} ->
             lthrift_process_lb:start(Args),
             _From ! done,
             create()
    end.
